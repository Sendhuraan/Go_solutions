...
(function() {
	...
	function SolutionConfig(DEFAULTS, solutionDir, commonConfigs, solutionConfigOptions) {
		...
		if(isNodeBundle) {
			...
			var isPlatformWindows = (os.platform() === 'win32');
			...
			var NODE_MAIN_FILE = (function(param, inputDir) {
				return `${inputDir}/${param}${isPlatformWindows ? '.exe' : ''}`;
			...
			var nodeBundleConfig = (function(config, entry, files, dependencies, outputDir, outputFile) {
				let output = `${outputDir}/${outputFile}${isPlatformWindows ? '.exe' : ''}`;
				var newConfig = Object.assign({}, config);

				newConfig.command = `go build -o ${output} ${entry}`;
